...
def søg_bevægelsesligningerne(acceleration, fra, til):
    ...
    middelacceleration = acceleration[1][start:slut]
    ...
    ax[1].plot([acceleration[0][start],acceleration[0][slut]],[np.mean(middelacceleration),np.mean(middelacceleration)],ls = '--',color = "#d62728",label = f"Gennemsnit: {np.mean(middelacceleration):.5f} m/s\u00B2")
